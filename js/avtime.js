...
// 广告过滤代理(去片头/片中广告), 通过站点配置 ext.proxy 指定, 如 http://192.168.0.2:12315/avtime_proxy.php
let PROXY = '';
...
async function init(cfg) {
    ...
    PROXY = (cfg && cfg.proxy) ? String(cfg.proxy).replace(/\/+$/, '') : '';
}
...
async function play(flag, id, flags) {
    ...
    if (pj && pj.url) {
        let playUrl = pj.url.replace(/\\/g, '');
        if (PROXY) playUrl = PROXY + '?url=' + encodeURIComponent(playUrl);
