# -*- coding: utf-8 -*-
# AI色情短剧 (91porna.com) 品牌合集采集
# 数据源：https://91porna.com/moviesets/brand/aiseqingduanju
import json
import re
import sys
import time
...
class Spider(Spider):
    ...
    NAME = 'AI色情短剧'
    BASE = '/moviesets/brand/aiseqingduanju'
    ...
    def homeContent(self, filter):
        raw = self._get(self.BASE)
        classes = [{'type_name': self.NAME, 'type_id': self.BASE}]
    ...
    def searchContent(self, key, quick, pg='1'):
        pg = int(pg or 1)
        kw = (key or '').strip()
        url = '/comic/index/search?keyword=' + quote(kw, safe='') if kw else self.BASE
        if pg > 1:
            sep = '&' if '?' in url else '?'
            url += sep + 'page=' + str(pg)
        try:
                ...
                for it in node.get('itemListElement', []):
                    ...
                    if not pic:
                        ...
                        if isinstance(th, str):
                            ...
                            pic = th[0]
                    date = item.get('datePublished') or item.get('uploadDate') or ''
                    out.append({'url': url, 'name': name, 'pic': pic, 'date': date})
                return out
